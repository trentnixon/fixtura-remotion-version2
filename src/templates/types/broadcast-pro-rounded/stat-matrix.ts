import type { BroadcastProRoundedScoreRole } from "./score-typography";

/** Layout tier for player performance stat presentation. */
export type BroadcastProRoundedStatMatrixTier =
  | "featuredTriple"
  | "gridTriple"
  | "performancesTriple"
  | "compact"
  | "resultRow";

export type BroadcastProRoundedStatLabelStyle = "full" | "short";

export interface BroadcastProRoundedStatMatrixCell {
  label?: string;
  value: string;
  highlight?: boolean;
}

export const BROADCAST_PRO_BATTING_STAT_LABELS = {
  full: { runs: "Runs", balls: "Balls", sr: "SR" },
  short: { runs: "Runs", balls: "Balls", sr: "SR" },
} as const;

export const BROADCAST_PRO_BOWLING_STAT_LABELS = {
  full: { figures: "Figures", overs: "Overs", economy: "Economy" },
  short: { figures: "Figs", overs: "Ov", economy: "Econ" },
} as const;

/** Theme `componentStyles` keys for triple-matrix layout by tier. */
export const BROADCAST_PRO_STAT_MATRIX_TRIPLE_TIER_KEYS: Record<
  Exclude<BroadcastProRoundedStatMatrixTier, "compact" | "resultRow">,
  {
    row: string;
    divider: string;
    label: string;
    scoreRole: Extract<
      BroadcastProRoundedScoreRole,
      "featuredStat" | "gridStat" | "performancesStat"
    >;
  }
> = {
  featuredTriple: {
    row: "broadcastProRoundedStatMatrixTripleFeatured",
    divider: "broadcastProRoundedStatMatrixDividerFeatured",
    label: "broadcastProRoundedStatMatrixLabel",
    scoreRole: "featuredStat",
  },
  gridTriple: {
    row: "broadcastProRoundedStatMatrixTripleGrid",
    divider: "broadcastProRoundedStatMatrixDividerGrid",
    label: "broadcastProRoundedStatMatrixLabel",
    scoreRole: "gridStat",
  },
  performancesTriple: {
    row: "broadcastProRoundedStatMatrixTriplePerformances",
    divider: "broadcastProRoundedStatMatrixDividerPerformances",
    label: "broadcastProRoundedStatMatrixLabelPerformances",
    scoreRole: "performancesStat",
  },
};

/** Teko score role for matrix value numerals. */
export const resolveBroadcastProRoundedStatMatrixScoreRole = (
  tier: BroadcastProRoundedStatMatrixTier,
): BroadcastProRoundedScoreRole => {
  if (tier === "compact") return "compactStat";
  if (tier === "resultRow") return "playerStatPrimary";
  return BROADCAST_PRO_STAT_MATRIX_TRIPLE_TIER_KEYS[tier].scoreRole;
};

/**
 * Tier-driven accent on stat values — performances highlight col 1;
 * result rows use explicit `cell.highlight`.
 */
export const resolveBroadcastProRoundedStatMatrixHighlight = (
  tier: BroadcastProRoundedStatMatrixTier,
  cellIndex: number,
  cell?: Pick<BroadcastProRoundedStatMatrixCell, "highlight" | "label">,
): boolean => {
  if (cell?.highlight) return true;
  if (tier === "performancesTriple" && cellIndex === 0) {
    return !!cell?.label;
  }
  return false;
};
