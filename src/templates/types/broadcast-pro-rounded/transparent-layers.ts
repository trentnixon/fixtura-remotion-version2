...
/** Broadcast Pro: sm / md / lg glass alpha presets (see resolveBroadcastProRoundedTransparentLayers). */
export type BroadcastProRoundedGlassOpacityPreset = "sm" | "md" | "lg";

export interface BroadcastProRoundedTransparentLayers {
  glass: {
...
export const DEFAULT_BROADCAST_PRO_TRANSPARENT_LAYERS: BroadcastProRoundedTransparentLayers =
  BROADCAST_PRO_TRANSPARENT_BY_PRESET.sm;

export function resolveBroadcastProRoundedTransparentLayers(opts: {
  ...
  if (opts.broadcastProRoundedTransparentLayers) {
    return opts.broadcastProRoundedTransparentLayers;
  }
  const preset = opts.broadcastProRoundedGlassOpacity ?? "md";
