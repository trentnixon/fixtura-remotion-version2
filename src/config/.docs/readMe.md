# Folder Overview

Application-level configuration for the Remotion video composition system. Intended for shared constants, layout defaults, routing mappings, and other cross-cutting config consumed by compositions, templates, and core utilities.

## Files

- (To be populated as config modules are added)

## Relations

- Parent folder: [../../README.md](../../README.md) (project root)
- Key dependencies: consumed by compositions, templates, core/utils
- Consumed by: routing, compositions (contentLayout, divideFixturesBy, timings), templates

## Dependencies

- Internal: none (config is leaf)
- External: none
