...
import ResultSingleDisplayScoreline from "./controller/ResultSingleDisplay/display-Scoreline";
import {
  TransitionDirection,
...
export const ResultSingleScoreline: React.FC = () => {
  ...
  const sequences = matchResults.map((match) => ({
    content: (
      <div className="h-full w-full" style={{ height: compositionHeight }}>
        <ResultSingleDisplayScoreline match={match} />
      </div>
    ),
