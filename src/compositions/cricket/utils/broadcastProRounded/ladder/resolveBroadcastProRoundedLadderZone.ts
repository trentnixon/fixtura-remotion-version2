import {
  ...
  type BroadcastProRoundedLadderZone,
  type BroadcastProRoundedLadderZoneResult,
  type BroadcastProRoundedLadderZoneSizing,
} from "../../../../../templates/types/broadcast-pro-rounded/ladder-zone";

export interface ResolveBroadcastProRoundedLadderZoneInput {
  position: number;
  index: number;
  totalTeams: number;
 ...
 */
export const resolveBroadcastProRoundedLadderZone = ({
  position,
  index,
  totalTeams,
  sizing = DEFAULT_BROADCAST_PRO_LADDER_ZONE_SIZING,
}: ResolveBroadcastProRoundedLadderZoneInput): BroadcastProRoundedLadderZoneResult => {
  ...
  const zone: BroadcastProRoundedLadderZone = "mid";
  return {
    zone,
    rowOpacity: 1,
    rankAccent: false,
