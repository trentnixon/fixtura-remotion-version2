...
export const PerformancesListCNSWPrivate: React.FC = () => {
  ...
  if (totalScreens <= 0) {
    console.warn(
      "[PerformancesListCNSWPrivate] No screens to display, returning placeholder",
    );
    return <NoPlayersData />;
  }

  // Debug logging (remove in production)
  console.log("[PerformancesListCNSWPrivate] Debug:", {
  ...
  const sequences = Array.from({ length: totalScreens }, (_, index) => ({
    content: (
      <PerformancesDisplayCNSWPrivate
  ...
  console.log("[PerformancesListCNSWPrivate] Sequences created:", {
    totalScreens,
...
// Export as CNSWPrivate for compatibility with original template
export const CNSWPrivate: React.FC = () => {
  return <PerformancesListCNSWPrivate />;
};
