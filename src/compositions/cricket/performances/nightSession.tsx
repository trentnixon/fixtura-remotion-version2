...
import PerformancesDisplayNightSession from "./controller/PerformancesDisplay/display-NightSession";
import NoPlayersData from "./modules/NoPlayersData/no-data";
import { transformPerformanceData } from "./utils/dataTransformer";

export const PerformancesListNightSession: React.FC = () => {
  ...
  const sequences = Array.from({ length: totalScreens }, (_, index) => ({
    content: (
      <PerformancesDisplayNightSession
...
export const nightSession: React.FC = () => {
  return <PerformancesListNightSession />;
};
